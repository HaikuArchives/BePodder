...
SplitPane::SplitPane(BRect frame, BView *one, BView *two,uint32 Mode):BView(frame, "", Mode,B_WILL_DRAW|B_FRAME_EVENTS){
   ...
   rotateH = LoadIcon("rotateH");
   mirrorH = LoadIcon("mirrorH");
   rotateV = LoadIcon("rotateV");
   mirrorV = LoadIcon("mirrorV");
   
   AddChild(rotate = new ImageButton(BRect(0,0,8,8),   "",new BMessage(ROTATE), NULL, NULL,NULL));
   AddChild(mirror = new ImageButton(BRect(5,5,14,14), "",new BMessage(MIRROR), NULL, NULL,NULL));
   
   rotate->DrawbackgroundColor(false);
...
void SplitPane::Draw(BRect /*f*/){
   ...
   if(align == B_VERTICAL){
      ...
      FillRect(BRect(pos.x+thickness.x/2-1,Bounds().top+pad.x+1,pos.x+thickness.x/2-1,Bounds().bottom-pad.x-1)); // 96
   }else{
      SetHighColor(255,255,255);// SetHighColor(145,145,145);
      FillRect(BRect(Bounds().left+pad.y+1,pos.y+thickness.y/2,Bounds().right-pad.y-1,pos.y+thickness.y/2)) ;// 145
   
     SetHighColor(145,145,145); //SetHighColor(96,96,96);
      FillRect(BRect(Bounds().left+pad.y+1,pos.y+thickness.y/2-1,Bounds().right-pad.y-1,pos.y+thickness.y/2-1)); // 96
   }
}
...
void SplitPane::Update(){
   ...
   if(p2){
	...
	//vertical-case!	
	if(align == B_VERTICAL){
 	rotate->MoveTo(pos.x + 1 , Bounds().top + pad.x + 1);
	mirror->MoveTo(pos.x + 1 , Bounds().top + pad.x + 16);
	rotate->SetBitmap(rotateV);
	mirror->SetBitmap(mirrorV);
   }else{
	rotate->MoveTo(Bounds().right-pad.y - 10, pos.y + 1);
	mirror->MoveTo(Bounds().right-pad.y - 25, pos.y + 1);
	rotate->SetBitmap(rotateH);
	mirror->SetBitmap(mirrorH);
   }
...
*******************************************************/
void SplitPane::MouseMoved(BPoint where,uint32  transition,const BMessage */*m*/){
   const BPoint & ms1 = swapPanes ? MinSizeTwo : MinSizeOne;
   const BPoint & ms2 = swapPanes ? MinSizeOne : MinSizeTwo;
