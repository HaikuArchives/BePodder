...
{
	BRect drawRect;
	rgb_color borderColor = mix_color(fMasterView->Color(B_COLOR_HEADER_BACKGROUND), make_color(0, 0, 0, 255), 128);
...
void TitleView::MouseDown(BPoint position)
{
	...
	if (buttons == B_SECONDARY_MOUSE_BUTTON && (fColumnFlags & B_ALLOW_COLUMN_POPUP)) {
		...
		for (int index = 0; index < fColumns->CountItems(); index++) {
			BColumn *column = (BColumn*) fColumns->ItemAt(index);
			if (!column) continue;
			if (column->ShowHeading() == false) continue;
			BString name;
...
void OutlineView::RedrawColumn(BColumn *column, float leftEdge, bool isFirstColumn)
{
	if (column) {
		...
		for (RecursiveOutlineIterator iterator(&fRows); iterator.CurrentRow();
			...
			if (line + rowHeight >= fVisibleRect.top) {
				...
				if (isFirstColumn) {
					//
					//debugger("First");
					//row->DrawBackground(sourceRect,fDrawBufferView,false,false);
		...
		//1			StrokeLine(BPoint(0, line + rowHeight - 2), BPoint(Bounds().Width(), line + rowHeight - 2));
		//RedrawColumn	StrokeLine(BPoint(0, line + rowHeight - 1), BPoint(Bounds().Width(), line + rowHeight - 1));
		...
		//2			StrokeLine(BPoint(0, line + rowHeight - 2), BPoint(Bounds().Width(), line + rowHeight - 2));
		//RedrawColumn	StrokeLine(BPoint(0, line + rowHeight - 1), BPoint(Bounds().Width(), line + rowHeight - 1));
			...
			for (int columnIndex = 0; columnIndex < numColumns; columnIndex++) {
				...
				if (fieldLeftEdge + column->Width() >= invalidBounds.left) {
...
#endif
					{
						...
						if (isFirstColumn) row->DrawBackground(fullRect,this,false,false);	
