...
//			printf("ooh. sit is %d and fSoftie_used is %d\n", sit, line->fSoftie_used);
        tr->Render(this,
          ...
          r);
        //debug: StrokeLine(BPoint (left, height),BPoint (left, height+2));
		}
...
*/
	  ...
	  BRect margin(
		...
		if(tr!=NULL) {
			...
			tr->MarginAfterTheText(this,view_color,margin);
			/*margin.left  = bounds.right-indent;
			margin.right = bounds.right;
			SetDrawingMode (B_OP_COPY);
      		//SetLowColor (view_color);
      		SetLowColor (255,0,0);
      		FillRect (margin,B_SOLID_LOW);
			*/
		}
		else {
      		// Margin after fText
...
void
RunView::BuildPopUp (BPoint point)
{
  ...
  fMyPopUp = new BPopUpMenu ("Context Menu", false, false);
...
bool					
RunView::CheckURL(BPoint point) //by xeD
{
	if (!fLine_count)
    	return false;
...
{
  if (CheckURL(point))
  		SetViewCursor (fURLCursor);   // if there aren't any URLs in the current line, go back to default
  else
  		SetViewCursor (B_CURSOR_SYSTEM_DEFAULT); // no URLs found, set back to default
  ...
  switch (transit)
  {
    ...
    case B_INSIDE_VIEW:
      ...
      if ((fTracking == 1) && (fSp_start != fSp_end))
      {
		...
		// add the text
		drag->AddData("text/plain", B_MIME_TYPE, fText.String(), 
			  	  fText.Length());
...
RunView::MessageReceived (BMessage *msg)
{
  switch (msg->what)
  {
	...
	if (be_clipboard->Lock())
      {
        BString fText;
      	msg->FindString("url",&fText);
      	
      	 BString map;
  		if(urls.FindString(fText.String(),&map) == B_OK)
  			fText.SetTo(map);
  		if(fText.IFindFirst("action:") == 0) return;	

        be_clipboard->Clear();

        BMessage *msgD (be_clipboard->Data());
        msgD->AddData ("text/plain", B_MIME_TYPE, fText.String(), fText.Length());
...
RunView::RecalcScrollBar (bool constrain)
{
  ...
  if (fLine_count)
   TextResized(BRect(0,value,0,fLines[fLine_count - 1]->fBottom));
  else
   TextResized(BRect(0,0,0,0));	


  if (fLine_count
  && (bounds.Contains (BPoint (0.0, 0.0)) == false
  ||  bounds.Contains (BPoint (0.0, fLines[fLine_count - 1]->fBottom + ExtraTextSize() )) == false))
  {
   
    bottom = fLines[fLine_count - 1]->fBottom + ExtraTextSize() + 5.0;
...
void					
RunView::AppendURL(const char * text,const char *url,int16 font){
	urls.AddString(text,url);
	Append(text,C_URL,C_URL,font);
}

void					
RunView::AppendURL(const char * text,const char * url,int16 back,int16 font){
	urls.AddString(text,url);
	Append(text,C_URL,back,font);
}
...
void RunView::LoadURL(const char *url) {
  ...
  if (argument.FindFirst ("://") == B_ERROR)
  {
    ...
    if (argument.IFindFirst ("www") == 0)
  	...
  	{	//by xeD
  		BString map;
  		if(urls.FindString(url,&map) == B_OK)
  			argument.SetTo(map);
 ...
 const char *args[] = { argument.String(), 0 };
  ...
  if (argument.IFindFirst ("action:") == 0)
  {
  	InvokeAction(argument.String());
  }
  else
   	if(fDefaultURLAction){
  		//printf("fDefaultURLAction\n");
  		fDefaultURLAction->Perform(BString(argument.String()));
  	}
  	else{
  		//printf("no fDefaultURLAction!\n");
...
//by xed
void
RunView::ClearActions(){
	actions.MakeEmpty();
}

void
RunView::AddAction(const char* symbolicName,RVAction* action){
	actions.AddPointer(symbolicName,(void*)action);
}

void					
RunView::SetDefaultOpenURLAction(RVAction* action){
	fDefaultURLAction=action;
}

RVAction*
RunView::FindAction(const char* symbolicName){
...
//should be private..
void
RunView::InvokeAction(BString mangledName){
	mangledName.RemoveFirst("action://");
